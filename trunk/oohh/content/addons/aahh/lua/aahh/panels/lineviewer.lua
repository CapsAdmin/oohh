...
function PANEL:GetTextSize(font, fontsize, text)
	return graphics.GetTextSize(font, text) * Vec2(1, 1) * fontsize
end
...
function PANEL:OnThink()
	if self.nextlayout and T >= self.nextlayout then
		self:DoLayout()
		return
	end
	if not self.nextupdate then self.nextupdate = T + 6 end
	if self.layoutfinished and T > self.nextupdate then
		self:DoLayout()
		self.nextupdate = T + 6
		return
	end
...
function PANEL:OnDraw()
	...
	if self:GetDrawBackground() then
		graphics.DrawFilledRect(Rect(0, 0, self:GetWidth(), self:GetHeight()), Color(0.1, 0.1, 0.1, 1))
	end
...
function PANEL:DrawYScrollbar()
	graphics.DrawFilledRect(self.ybar, Color(1, 1, 1, 0.35 + (self.ybarhighlight and 0.2 or 0)))
end

function PANEL:DrawLine(line)
	if line.visible and self:GetDrawBackground() then --and #line.children > 0 then
		graphics.DrawFilledRect(line.rect-Rect(self.viewx, self.viewy, 0, 0), line.color)
	end
end
...
function PANEL:DrawText(element)
	...
	if element.struct.bgcolor then
		graphics.DrawFilledRect(element.rect-Rect(self.viewx, self.viewy, 0, 0), element.struct.bgcolor)
	end
	
	--local center = element.rect:GetPos() + element.rect:GetSize()/2
	--graphics.DrawText(element.text, center, "default", 12, Color(1, 1, 1, 1))
	...
	if element.struct.fgcolor then
		...
		surface.DrawText(
			...
			textr.x-self.viewx,
			textr.y-self.viewy,
			Vec2(fontsize, fontsize) / Vec2(render.GetScreenScale()) * 1.75,
			nil,
			nil,
			2
		)
