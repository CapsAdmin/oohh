...
LUAMTA_FUNCTION(material, SetShader)
{
	auto self = my->ToMaterial(1);
	auto name = my->ToString(2);

	auto shader = gEnv->pRenderer->EF_LoadShaderItem(name, false);

	if (!shader.m_pShader || shader.m_pShader->GetFlags() & EF_NOTFOUND)
	{
		my->Push(false);
		my->Push("failed to load shader");

		return 2;
	}

	self->AssignShaderItem(shader);

	return 0;
}

LUAMTA_FUNCTION(material, GetShader)
{
	auto self = my->ToMaterial(1);

	my->Push(self->GetShaderItem().m_pShader->GetName());

	return 1;
}

LUAMTA_FUNCTION(material, SetTexture)
{
	auto self = my->ToMaterial(1);
	...
	if (res)
	{
		auto val = res->GetTexture(my->ToNumber(2, EFTT_DIFFUSE));
		
		if (val)
		{
			SAFE_RELEASE(val->m_Sampler.m_pITex);

			val->m_Sampler.m_pTex = (CTexture *)my->ToTexture(3);
		}
	}
