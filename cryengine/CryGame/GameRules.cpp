...
void CGameRules::PostInit( IGameObject * pGameObject )
{
	...
	my->CallHook("GamePostInitialize");
}

//------------------------------------------------------------------------
void CGameRules::InitClient(int channelId)
{
	my->CallHook("GameInitialize", channelId);
}
...
void CGameRules::PostInitClient(int channelId)
{
	...
	my->CallHook("PostInit", channelId);
}
...
void CGameRules::OnConnect(struct INetChannel *pNetChannel)
{
	...
	my->CallHook("ClientConnect", pNetChannel->GetName());
...
void CGameRules::OnDisconnect(EDisconnectionCause cause, const char *desc)
{
	...
	my->CallHook("ClientDisconnect", cause, desc);
...
bool CGameRules::OnClientConnect(int channelId, bool isReset)
{
	...
	if (pActor)
	{
		...
		my->CallHook("PlayerEntered", pActor);
...
void CGameRules::OnClientDisconnect(int channelId, EDisconnectionCause cause, const char *desc, bool keepClient)
{
	...
	my->CallHook("PlayerLeft", pActor, cause, desc, keepClient);
...
{
	auto actor = oohh::GetEntityFromId(actorId);
	auto item = oohh::GetEntityFromId(itemId);

	my->CallHook("ItemDropped", actor, item);
	my->CallEntityHook(actor, "OnItemDropped", item);
	my->CallEntityHook(item, "OnDrop", actor);
...
void CGameRules::OnItemPickedUp(EntityId itemId, EntityId actorId)
{
	...
	my->CallHook("ItemPickedUp", actor, item);
	my->CallEntityHook(actor, "OnItemPickedUp", item);
	my->CallEntityHook(item, "OnPickUp", actor);
...
{
	my->CallHook("VehicleDestroyed", oohh::GetEntityFromId(id));
...
{
	my->CallHook("VehicleSubmerged", oohh::GetEntityFromId(id), ratio);
...
void CGameRules::RevivePlayer(CActor *pActor, const Vec3 &pos, const Ang3 &angles, int teamId, bool clearInventory)
{
	...
	my->CallHook("PlayerRespawn", pActor,angles, teamId, clearInventory);
}
