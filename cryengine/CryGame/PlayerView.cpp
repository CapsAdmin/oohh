...
{
	if (my && my->CallHook("PlayerPreViewProcess", rPlayer.GetEntity(), viewParams.position, viewParams.rotation, viewParams.fov, 3))
	{
		if (my->IsVec3(1))
			viewParams.position = my->ToVec3(1);
		
		if (my->IsQuat(2))
			viewParams.rotation = my->ToQuat(2);

		if (my->IsNumber(3))
			viewParams.fov = my->ToNumber(3);
	}
...
void CPlayerView::ViewProcess(SViewParams &viewParams)
{
	...
	if (my && false)
	{
		my->CallHook("PlayerViewProcess", viewParams.position, viewParams.rotation, viewParams.fov, 3);

		viewParams.position = my->ToVec3(-3, viewParams.position);
		viewParams.rotation = my->ToQuat(-2, viewParams.rotation);
		viewParams.fov = my->ToNumber(-1, viewParams.fov);
	}
